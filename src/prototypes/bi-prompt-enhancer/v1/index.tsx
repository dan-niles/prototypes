...
import {
    ...
    Loader2, Check, ArrowRight, ChevronLeft, ChevronRight, Zap
} from 'lucide-react';
...
export default function AI_PromptEditor() {
    const [appState, setAppState] = useState<AppState>('IDLE');

    // Versioning State: Index 0 is always the baseline/original.
    ...
    const handleEnhanceSubmit = (instruction: string) => {
        if (!instruction.trim()) instruction = "Auto-Enhance"; // Default fallback
        setAppState('LOADING');
    ...
    const handleDiscard = () => {
        // Revert completely to original text and clear history
    ...
    const handleAccept = () => {
        // In a real app, this confirms the save. We just drop out of review mode.
        setAppState('IDLE');
    };
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-gray-300 p-8 flex items-center justify-center font-sans">
            ...
            <div className={`w-full max-w-5xl bg-[#141414] border ${appState === 'REVIEW' ? 'border-blue-900/50 shadow-blue-900/10' : 'border-[#2a2a2a]'} rounded-xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300`}>
                ...
                <div className={`flex items-center gap-1 px-4 py-2 border-b border-[#2a2a2a] bg-[#181818] relative z-20 ${appState === 'LOADING' ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <button className="p-1.5 text-gray-400 hover:text-white hover:bg-[#2a2a2a] rounded"><Link size={16} /></button>
                    <div className="w-px h-5 bg-[#333] mx-2" />

                    {/* Wand Button & Anchored Popover Wrapper */}
                    <div className="relative">
                        <button
                            onClick={() => setAppState(appState === 'POPOVER_OPEN' ? 'IDLE' : 'POPOVER_OPEN')}
                            disabled={appState === 'REVIEW' && isOriginal}
                            className={`p-1.5 rounded flex items-center gap-1.5 transition-all ${appState === 'POPOVER_OPEN'
                                ? 'bg-blue-600/20 text-blue-400 ring-1 ring-blue-500/50'
                                : 'text-gray-400 hover:text-blue-400 hover:bg-[#2a2a2a]'
                                }`}
                        >
                            <Wand2 size={16} />
                        </button>
                        ...
                        {appState === 'POPOVER_OPEN' && (
                            <div ref={popoverRef} className="absolute top-full left-0 mt-2 w-72 bg-[#1e1e1e] border border-[#333] rounded-lg shadow-2xl shadow-black overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150 z-50 origin-top-left">
                                ...
                                {/* Zero-Click Auto-Enhance */}
                                <div className="p-1.5 border-b border-[#333] bg-[#1a1a1a]">
                                    <button
                                        onClick={() => handleEnhanceSubmit("Auto-Enhance")}
                                        className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-blue-100 bg-blue-600/10 hover:bg-blue-600 hover:text-white rounded-md transition-colors"
                                    >
                                        <Sparkles size={16} className="text-blue-400" /> Auto-Enhance
                                    </button>
                                </div>

                                {/* Specific Strategies */}
                                <div className="p-1.5 border-b border-[#333]">
                                    <span className="px-3 py-1 text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Strategies</span>
