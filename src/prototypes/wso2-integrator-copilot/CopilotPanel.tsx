            ...
            {showSettings ? (
                <div className="flex-1 flex flex-col overflow-y-auto">
                    ...
                    <div className="p-5 space-y-6">
                        ...
                        {headerMode === 'mi' && (
                            <>
                                ...
                                <div className="space-y-3">
                                    <h3 className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Behavior</h3>
                                    <div className="space-y-4">
                                        {/* Intelligence */}
                                        <div className="space-y-1.5">
                                            <p className="text-[12.5px] font-medium text-gray-700">Intelligence</p>
                                        ...
                                        {/* Sub-agent */}
                                        <div className="space-y-1.5">
                                            <p className="text-[12.5px] font-medium text-gray-700">Sub-agent</p>
                                            <div className="flex bg-gray-100 rounded-lg p-0.5 border border-gray-200/50">
                                                {['normal', 'high'].map((level) => (
                                                    <button
                                                        key={level}
                                        ...
                                        <div className="flex items-center justify-between pt-1">
                                            <p className="text-[12.5px] font-medium text-gray-700">Extended thinking</p>
                                            <div className="flex bg-gray-100 rounded-lg p-0.5 border border-gray-200/50">
                                                <button
                        ...
                        {/* Capabilities */}
                        <div className="space-y-3">
                            <h3 className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Capabilities</h3>
                            <div className="border border-gray-200 rounded-lg overflow-hidden divide-y divide-gray-100">
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[11.5px] text-gray-400 mt-0.5">{skillsTotal} skills</p>
                                    </div>
                                    <ChevronRight size={16} strokeWidth={1.5} className="text-gray-300 shrink-0" />
                                </button>
                            </div>
                        ...
                        {/* Inline completions */}
                        <div className="space-y-3">
                            <h3 className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Inline completions</h3>
                            <div className="border border-gray-200 rounded-lg p-3">
                                <div className="flex items-center justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className="text-[13px] font-medium text-gray-900">GitHub Copilot</p>
                                    ...
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-md text-[12px] font-medium transition-colors shrink-0 ${githubAuthorized ? 'text-green-700 bg-green-50 border border-green-200' : 'text-white bg-blue-600 hover:bg-blue-700'}`}
                                    >
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="border border-gray-200 rounded-lg p-3">
                                <div className="flex items-center justify-between gap-3">
                                    ...
                                    <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12px] font-medium text-red-600 bg-red-50 border border-red-200 hover:bg-red-100 transition-colors shrink-0">
...
const INITIAL_SKILLS: Skill[] = [
    {
        id: 'openapi-import',
        name: 'openapi-import',
        description: 'Scaffold WSO2 integration services from an OpenAPI specification.',
        source: 'Built-in',
        addedBy: 'WSO2',
        updatedAt: 'Apr 02, 2026',
        enabled: true,
        content: `# openapi-import

Scaffold WSO2 integration services from an OpenAPI specification.

## When to use this skill

- The user provides or references an OpenAPI spec (\`.yaml\` / \`.json\`)
- The user asks to scaffold a service from a spec
- The user wants to import endpoints from an existing API

## What it does

1. Parses the OpenAPI document and validates it
2. Generates a Ballerina service skeleton with one resource per path
3. Creates request/response record types from the spec's schemas
4. Wires basic input validation and error responses

## Output

A \`service.bal\` (and supporting \`types.bal\`) ready to run with \`bal run\`.`,
    },
    {
        id: 'data-mapper',
        name: 'data-mapper',
        description: 'Generate type-safe data transformations between record types.',
        source: 'Built-in',
        addedBy: 'WSO2',
        updatedAt: 'Apr 02, 2026',
        enabled: true,
...
Generate type-safe data transformations between record types.

## When to use

- Mapping between two record types (e.g. inbound payload to outbound model)
- Translating between protocols or schemas

## How it works

Provide source and target types. The skill generates a Ballerina mapping function with field-by-field assignments. Default mappings are inferred by field name and type; ambiguous fields are flagged for review.`,
    },
    {
        id: 'integration-patterns',
        name: 'integration-patterns',
        description: 'Apply enterprise integration patterns (EIP) idiomatically in Ballerina.',
        source: 'Built-in',
        addedBy: 'WSO2',
        updatedAt: 'Apr 02, 2026',
        enabled: true,
...
Apply enterprise integration patterns idiomatically in Ballerina.

## Patterns covered

- **Content-based routing** — dispatch by message attribute
- **Aggregator** — collect related messages into one
- **Splitter** — fan out one message into many
- **Scatter-gather** — call N services in parallel and merge
- **Dead letter channel** — quarantine bad messages

## When to use

When the user describes a flow involving routing, fan-out/fan-in, or error quarantine. The skill picks the right pattern and generates the Ballerina implementation.`,
    },
    {
        id: 'skill-creator',
        name: 'skill-creator',
...
- Prompts for a name, description, and scope (project / user)
- Generates a new skill folder with a \`SKILL.md\` and example logic
- Registers the skill so it appears in the Skills page and Tools picker`,
    },
    {
        id: 'connector-scaffolding',
        name: 'connector-scaffolding',
        description: 'Scaffold a new WSO2 connector module from a service description.',
        source: 'Custom',
        addedBy: 'You',
        updatedAt: 'May 20, 2026',
        enabled: true,
        content: `# connector-scaffolding

Scaffold a new WSO2 connector module from a service description.

Used when the team needs a new integration with an external service that isn't in the connector catalog yet. Creates the module structure, configures auth flows, and stubs out the resource functions.

## When to use

- Adding a new SaaS integration to the project
- Replacing an ad-hoc HTTP client with a proper connector module
- Generating a starting point for a vendor SDK wrapper`,
    },
    {
        id: 'eip-patterns',
        name: 'eip-patterns',
        description: 'Project-specific EIP guidance — refers to our shared utilities under lib/eip/.',
        source: 'Custom',
        addedBy: 'You',
        updatedAt: 'May 20, 2026',
        enabled: true,
        content: `# eip-patterns

Project-specific guidance for applying enterprise integration patterns in this codebase.

Refers to our shared routing, aggregation, and DLQ utilities under \`lib/eip/\`. Use this skill before introducing new pattern implementations to ensure we reuse the existing helpers.

## Helpers

- \`lib/eip/router.bal\` — content-based routing primitives
- \`lib/eip/aggregator.bal\` — message correlation and aggregation
- \`lib/eip/dlq.bal\` — dead-letter quarantine + replay helpers`,
    },
    {
        id: 'fhir-r4',
        name: 'fhir-r4',
        description: 'FHIR R4 conventions, validation profiles, and reference resolution helpers for this project.',
        source: 'Custom',
        addedBy: 'You',
        updatedAt: 'May 20, 2026',
        enabled: true,
...
FHIR R4 conventions and helpers used by this project.

- Resource validation against our locked-down profile set in \`profiles/\`
- Reference resolution helpers in \`lib/fhir/refs.bal\`
- Bundle batching rules for ingestion

Apply this skill when generating any FHIR-related resource or transformation.`,
    },
    {
        id: 'ccda-documents',
        name: 'ccda-documents',
        description: 'CCDA document parsing and emission rules — section ordering, narrative blocks, and our extensions.',
        source: 'Custom',
        addedBy: 'You',
        updatedAt: 'May 20, 2026',
        enabled: true,
        content: `# ccda-documents

CCDA document parsing and emission rules for this integration.

Handles section ordering, narrative blocks, and our specific extensions. See \`docs/ccda.md\` for the full reference.`,
    },
    {
        id: 'saml-auth',
        name: 'saml-auth',
        description: 'SAML 2.0 patterns standardized across our services — IdP/SP flows, assertion validation, JWT bridging.',
        source: 'Custom',
        addedBy: 'You',
        updatedAt: 'May 20, 2026',
        enabled: true,
...
SAML 2.0 authentication patterns standardized across our services.

- IdP-initiated and SP-initiated flows
- Assertion validation against our trusted issuers
- Session bridging into our gateway's JWT token`,
    },
];
...
function SkillsPage({ skills, setSkills, onBack }: {
    ...
    const viewing = viewingId ? skills.find((s) => s.id === viewingId) : null;
    const remove = (id: string) => setSkills((prev) => prev.filter((s) => s.id !== id));

    if (viewing) {
        const isCustom = viewing.source === 'Custom';
        return (
            <SkillDetailView
                skill={viewing}
    ...
    const toggleCollapsed = (key: string) => setCollapsed((prev) => { const n = new Set(prev); n.has(key) ? n.delete(key) : n.add(key); return n; });
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-5">
                ...
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <button onClick={() => toggleCollapsed('custom')} className="flex items-center gap-1.5 text-gray-500 hover:text-gray-800 transition-colors">
                            {isOpen('custom') ? <ChevronDown size={13} strokeWidth={2.5} /> : <ChevronRight size={13} strokeWidth={2.5} />}
                            <span className="text-[11px] font-semibold uppercase tracking-wider">Custom skills</span>
                            <span className="text-[11px] text-gray-300 font-medium">{custom.length}</span>
                        </button>
                        <Switch checked={customAllOn} onChange={toggleAllCustom} disabled={custom.length === 0} />
                    </div>
                    <p className="text-[11.5px] text-gray-400 mb-2.5 ml-[19px]">Skills you've added to this project.</p>
                    <Collapse open={isOpen('custom')}>
                        {custom.length === 0 ? (
                            <p className="text-[12px] text-gray-400 italic ml-[19px]">No custom skills yet.</p>
                        ) : (
                            <div className="border border-gray-200 rounded-lg overflow-hidden divide-y divide-gray-100">
                                {custom.map((s) => (
                                    <SkillRow key={s.id} skill={s} onToggle={() => toggleSkill(s.id)} onOpen={() => setViewingId(s.id)} onEdit={() => { }} onRemove={() => remove(s.id)} />
                                ))}
                            </div>
                        )}
                    </Collapse>
        ...
        <div className="group/row">
            <div className="flex items-start gap-2.5 px-3 py-2.5 hover:bg-gray-50 transition-colors">
                <button onClick={onOpen} className="flex-1 min-w-0 text-left">
                    <p className={`text-[13px] font-medium truncate ${skill.enabled ? 'text-gray-900' : 'text-gray-400'}`}>{skill.name}</p>
                    <p className="text-[11.5px] text-gray-500 mt-0.5 truncate">{skill.description}</p>
                </button>
                {(onEdit || onRemove) && (
                    <div className="hidden group-hover/row:flex items-center gap-0.5 shrink-0 mt-0.5">
                        {onEdit && (
                            <button onClick={onEdit} title="Edit" className="flex items-center justify-center w-6 h-6 rounded text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors">
                                <Pencil size={12} strokeWidth={2} />
                            </button>
                        )}
                        {onRemove && (
                            <button onClick={onRemove} title="Delete" className="flex items-center justify-center w-6 h-6 rounded text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors">
    ...
    return (
        <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 shrink-0">
                ...
                <span className="text-[14px] font-semibold text-gray-900 truncate flex-1">{skill.name}</span>
                {onEdit && (
                    <button onClick={onEdit} title="Edit skill" className="flex items-center justify-center w-7 h-7 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                    <div>
                        <p className="text-[11px] text-gray-400 mb-0.5">Added by</p>
                        <p className="text-[13px] text-gray-800">{skill.addedBy}</p>
                    </div>
                    <div>
                        <p className="text-[11px] text-gray-400 mb-0.5">Last updated</p>
                        <p className="text-[13px] text-gray-800">{skill.updatedAt}</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p className="text-[13px] text-gray-700 leading-relaxed">{skill.description}</p>
                </div>
...
function SkillMarkdown({ children }: { children: string }) {
    return (
        <div className="text-[13px] text-gray-700 leading-relaxed">
            <ReactMarkdown
                components={{
                    h1: ({ children }) => <h1 className="text-[18px] font-bold text-gray-900 mt-1 mb-3">{children}</h1>,
                    h2: ({ children }) => <h2 className="text-[14px] font-semibold text-gray-900 mt-5 mb-2">{children}</h2>,
                    h3: ({ children }) => <h3 className="text-[13px] font-semibold text-gray-800 mt-4 mb-1.5">{children}</h3>,
                    p: ({ children }) => <p className="mb-3 leading-relaxed">{children}</p>,
                    ul: ({ children }) => <ul className="list-disc pl-5 mb-3 space-y-1">{children}</ul>,
                    ol: ({ children }) => <ol className="list-decimal pl-5 mb-3 space-y-1">{children}</ol>,
                    li: ({ children }) => <li className="leading-relaxed">{children}</li>,
                    code: ({ children }) => <code className="bg-gray-100 px-1.5 py-0.5 rounded text-[12px] font-mono text-gray-700">{children}</code>,
                    pre: ({ children }) => <pre className="bg-gray-50 border border-gray-200 rounded-md p-3 overflow-x-auto text-[12px] font-mono mb-3">{children}</pre>,
...
function AddSkillForm({ onBack, onAdd }: { onBack: () => void; onAdd: (s: Skill) => void }) {
    ...
    const submit = () => {
        ...
        onAdd({
            ...
            content: instructions.trim() || `# ${name.trim()}\n\n${description.trim() || 'Custom skill.'}`,
            source: 'Custom',
            addedBy: 'You',
